...
class VlogFileSink : public absl::LogSink {
 public:
  VlogFileSink(FILE* filep) : filep_(filep) { absl::AddLogSink(this); }
  ...
  void Send(const absl::LogEntry& entry) final {
    FILE* fp = entry.verbosity() == absl::LogEntry::kNoVerbosityLevel ? stderr
                                                                      : filep_;
    fprintf(fp, "%s", entry.text_message_with_prefix_and_newline_c_str());
  }
...
void SetVlogFilenameFromEnv() {
  const char* env = getenv("TF_CPP_VLOG_FILENAME");
  if (env == nullptr) return;
  FILE* filep = fopen(env, "w");
  if (filep == nullptr) return;

  static const absl::NoDestructor<VlogFileSink> file_sink(filep);
